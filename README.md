...
- ```AND (rating * 2) = FLOOR(rating * 2)``` makes sure the ratings are in **0.5 increments**. ```(rating * 2)``` multiplies the rating by **2**, ```=``` or **equality operator** compares these **mathematical expressions**, and ```FLOOR(rating * 2)``` in ```FLOOR(x)``` rounds down to the nearest whole number. For example: (6.7 * 2) = 13.4 and FLOOR(6.7 * 2) = 13, and therefore 13.4 = 13 is **not true** and will fail the check. However, for example: (6.5 * 2) = 13 and FLOOR(6.5 * 2) = 13, and therefore 13 = 13 is **true** and allowed.
